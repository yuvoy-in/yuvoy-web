...
- **The site owns the notch (2026-08-11):** the viewport declares `viewport-fit=cover`, so pages always extend under the Dynamic Island and `env(safe-area-inset-*)` is live. Every safe-area offset is written as `max(<old value>, env(...))` or `calc(<old value> + env(...))` — on hardware without insets the layout is unchanged by construction, which is also why the e2e geometry suite needs no special casing. `container-page` floors its gutters against the side insets for landscape phones.
